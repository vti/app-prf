...
sub list_deps : Test {
    ...
    $self->_mkfile('script.pl', q/use Time::Local;/);
    $self->_mkfile('Main.pm', q/package Main; use base 'Foo::Bar::Baz'; sub new {} 1;/);
    $self->_mkfile('Main2.pm', q/package Main; use parent 'Foo::Bar::Baz'; sub new {} 1;/);
...
sub _build_command {
    ...
    return App::PRF::Command::list_deps->new(color => 0, @_);
}
...
sub _create_root {
    ...
    my $root =
      File::Spec->catfile(File::Basename::dirname(__FILE__), '../list_deps');
    $self->{root} = $root;
